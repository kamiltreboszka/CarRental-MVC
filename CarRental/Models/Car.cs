...
namespace CarRental.Models
{
    public class Car: ICarTesting
    {
        ...
        public void CarTesting(string CarDetails)
        {
            Regex carDetailsRegex = new Regex(@"^(?<Marka>\D+)\s+(?<Model>\D+)\s+(?<Rok>(\w|\/)+)\s+(?<Paliwo>\D+)\s+(?<CenaDobowa>(\w|\/)+)\b", RegexOptions.Compiled);
            var matches = carDetailsRegex.Matches(CarDetails);
            foreach(Match match in matches)
            {
                var groups = match.Groups;
                this.Marka = groups["Marka"].Value;
                this.Model = groups["Model"].Value;
                this.Rok = groups["Rok"].Value;
                this.Paliwo = groups["Paliwo"].Value;
                this.CenaDobowa =  Double.Parse(groups["CenaDobowa"].Value);
            }
        }
    }
}